...
class app(object):
    ...
    def create_migrate_entity(self, entity_type:str, entity_id:str, main_gate_name:str, main_conn_id:str, gates:list[str], hubs:list[str], argvs: dict):
        _creator = self.__entity_migrate_method__[entity_type]
        _entity = _creator(entity_id, main_gate_name, main_conn_id, gates, hubs, argvs)
    ...
    def tick(self, update:Callable[[], None]):
        update()
        _t = Timer(0.016, lambda : self.tick(update))
        _t.start()
